...
int _main(int argc, char *argv[])
{
    ...
    maptab.add(Segment("seg_kunc", SEG_KUNC_BASE, SEG_KUNC_SIZE, IntTab(TGTID_RAM), false)); // Kernel uncacheable
    ...
    proc = new VciXcacheWrapper<vci_param, Mips32ElIss>("mips32", 0, maptab, IntTab(0), icache_ways, icache_sets, icache_words, dcache_ways, dcache_sets, dcache_words);

    VciSimpleRam<vci_param> *rom;
    rom = new VciSimpleRam<vci_param>("rom", IntTab(TGTID_ROM), maptab, loader);

    VciSimpleRam<vci_param> *ram;
    ram = new VciSimpleRam<vci_param>("sram", IntTab(TGTID_RAM), maptab, loader);

    VciMultiTty<vci_param> *tty;
    tty = new VciMultiTty<vci_param>("tty", IntTab(TGTID_TTY), maptab, "ma_poule", NULL);

    VciGcdCoprocessor<vci_param> *gcd;
    gcd = new VciGcdCoprocessor<vci_param>("gcd", IntTab(TGTID_GCD), maptab);

    VciVgsb<vci_param> *bus;
    bus = new VciVgsb<vci_param>("vci_bus", maptab, 1, 4);
