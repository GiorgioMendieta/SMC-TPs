    ...
    namespace caba
    {
        ...
        namespace
        {
            const char* dcache_fsm_state_str[] = {
                "DCACHE_IDLE",      "DCACHE_WRITE_UPDT", "DCACHE_WRITE_REQ ", "DCACHE_MISS_SELECT", "DCACHE_MISS_INVAL",
                "DCACHE_MISS_WAIT", "DCACHE_UNC_WAIT",   "DCACHE_XTN_HIT",    "DCACHE_XTN_INVAL",   "DCACHE__XTN_SYNC",
            };
            const char* icache_fsm_state_str[] = {
                "ICACHE_IDLE     ", "ICACHE_MISS_SELECT", "ICACHE_MISS_INVAL", "ICACHE_MISS_WAIT", "ICACHE_UNC_WAIT ",
            };
            const char* cmd_fsm_state_str[] = {
                "CMD_IDLE      ", "CMD_INS_MISS  ", "CMD_INS_UNC   ",
                "CMD_DATA_MISS ", "CMD_DATA_UNC  ", "CMD_DATA_WRITE",
            };
            const char* rsp_fsm_state_str[] = {
                "RSP_IDLE      ", "RSP_INS_MISS  ", "RSP_INS_UNC   ",
                "RSP_DATA_MISS ", "RSP_DATA_UNC  ", "RSP_DATA_WRITE",
            };
        } // namespace
...
#define tmpl(...) template <typename vci_param, typename iss_t> __VA_ARGS__ VciXcacheWrapperAdvanced<vci_param, iss_t>
        ...
        tmpl(/**/)::VciXcacheWrapperAdvanced(sc_module_name name, int proc_id, const soclib::common::MappingTable& mt,
                                             ...
                                             size_t dcache_sets, size_t dcache_words, size_t wbuf_nwords,
                                             size_t wbuf_nlines) :
            soclib::caba::BaseModule(name),

            p_clk("clk"), p_resetn("resetn"), p_vci("vci"),

            m_cacheability_table(mt.getCacheabilityTable()), m_srcid(mt.indexForId(index)),

            m_dcache_ways(dcache_ways), m_dcache_words(dcache_words),
            m_dcache_yzmask((~0) << (uint32_log2(dcache_words) + 2)), m_icache_ways(icache_ways),
            m_icache_words(icache_words), m_icache_yzmask((~0) << (uint32_log2(icache_words) + 2)),

            m_iss(this->name(), proc_id),

            r_dcache_fsm("r_dcache_fsm"), r_dcache_addr_save("r_dcache_addr_save"),
            r_dcache_wdata_save("r_dcache_wdata_save"), r_dcache_type_save("r_dcache_type_save"),
            r_dcache_be_save("r_dcache_be_save"), r_dcache_cacheable_save("r_dcache_cacheable_save"),
            r_dcache_way_save("r_dcache_way_save"), r_dcache_set_save("r_dcache_set_save"),
            r_dcache_word_save("r_dcache_word_save"), r_dcache_miss_req("r_dcache_miss_req"),
            r_dcache_unc_req("r_dcache_unc_req"),

            r_icache_fsm("r_icache_fsm"), r_icache_addr_save("r_icache_addr_save"),
            r_icache_miss_req("r_icache_miss_req"), r_icache_unc_req("r_icache_unc_req"),

            r_vci_cmd_fsm("r_vci_cmd_fsm"), r_vci_cmd_min("r_vci_cmd_min"), r_vci_cmd_max("r_vci_cmd_max"),
            r_vci_cmd_cpt("r_vci_cmd_cpt"),

            r_vci_rsp_fsm("r_vci_rsp_fsm"), r_vci_rsp_ins_error("r_vci_rsp_ins_error"),
            r_vci_rsp_data_error("r_vci_rsp_data_error"), r_vci_rsp_cpt("r_vci_rsp_cpt"),

            r_vci_rsp_fifo_ins("r_vci_rsp_fifo_ins", 2), r_vci_rsp_fifo_data("r_vci_rsp_fifo_data", 2),
            ...
            r_dcache("dcache", dcache_ways, dcache_sets, dcache_words)
        {
            ...
            assert((vci_param::T > 2) && ((1 << (vci_param::T - 1)) >= wbuf_nlines) && "I need more TRDID bits");
        ...
        tmpl(void)::cache_monitor(addr_t addr)
        {
            ...
            // data cache
            cache_hit = r_dcache.read_neutral(addr, &cache_rdata, &cache_way, &cache_set, &cache_word);

            if ((cache_hit != m_debug_previous_d_hit) or (cache_hit and (cache_rdata != m_debug_previous_d_rdata)))
            {
                std::cout << "Monitor Proc " << name() << " DCACHE at cycle " << std::dec << m_cpt_total_cycles
                          << " / HIT = " << cache_hit << " / ADR = " << std::hex << addr << " / DATA = " << cache_rdata
            ...
            // icache monitor
            cache_hit = r_icache.read_neutral(addr, &cache_rdata, &cache_way, &cache_set, &cache_word);

            if ((cache_hit != m_debug_previous_i_hit) or (cache_hit and (cache_rdata != m_debug_previous_i_rdata)))

            {
                std::cout << "Monitor Proc " << name() << " ICACHE at cycle " << std::dec << m_cpt_total_cycles
                          << " / HIT = " << cache_hit << " / ADR = " << std::hex << addr << " / DATA = " << cache_rdata
                          << " / WAY = " << cache_way << std::endl;
        ...
        tmpl(void)::print_trace(size_t mode)
        {
            ...
            std::cout << "  " << icache_fsm_state_str[r_icache_fsm.read()] << " | "
                      << dcache_fsm_state_str[r_dcache_fsm.read()] << " | " << cmd_fsm_state_str[r_vci_cmd_fsm.read()]
        ...
        tmpl(void)::transition()
        {
            ...
            case ICACHE_MISS_SELECT: // selects a slot in an associative set
            {
                ...
                valid             = r_icache.victim_select(r_icache_addr_save.read(), &victim, &way, &set);
            ...
            case ICACHE_MISS_INVAL: // invalidate the selected slot
            {
                ...
                r_icache.inval(r_icache_way_save.read(), r_icache_set_save.read(), &nline);
            ...
            case ICACHE_MISS_WAIT: // wait response and update dcache
            {
                ...
                else if (r_vci_rsp_fifo_ins.rok()) // available instruction
                {
                    ...
                    r_icache.write(r_icache_way_save.read(), r_icache_set_save.read(), r_icache_word_save.read(),
                                   r_vci_rsp_fifo_ins.read());

                    if (r_icache_word_save.read() == m_icache_words - 1) // last word
                    {
                        r_icache.victim_update_tag(r_icache_addr_save.read(), r_icache_way_save.read(),
            ...
            case DCACHE_IDLE: {
                if (m_dreq.valid)
                {
                    ...
                    else if ((m_dreq.type == iss_t::DATA_LL) or (m_dreq.type == iss_t::DATA_SC))
            ...
            case DCACHE_MISS_SELECT: // select a slot in an associative set
            {
                ...
                valid             = r_dcache.victim_select(r_dcache_addr_save.read(), &victim, &way, &set);
            ...
            case DCACHE_UNC_WAIT: // wait the response
            {
                ...
                {
                    vci_rsp_fifo_data_get = true;
                    if (m_dreq.valid and (m_dreq.addr == r_dcache_addr_save.read())) // request un modified
                    {
                        m_drsp.valid = true;
            ...
            case DCACHE_XTN_HIT: // checking hit for an XTN_INVAL request
            {
                ...
                size_t way;
                size_t set;
                bool hit = r_dcache.read(r_dcache_wdata_save.read(), &data, &way, &set, &word);
            ...
            if (m_ireq.valid and m_irsp.valid and (not m_dreq.valid or m_drsp.valid) and (m_ireq.addr != m_pc_previous))
            {
                m_cpt_exec_ins++;
            ...
            case RSP_DATA_WRITE: {
                if (p_vci.rspval.read())
                {
                    ...
                    r_vci_rsp_fsm = RSP_IDLE;
                    TO BE COMPLETED if ((p_vci.rerror.read() & 0x1) == 0x1) m_iss.setWriteBerr();
                }
                break;
            }
            //////////////////
            case RSP_INS_MISS: {
                if (p_vci.rspval.read())
                {
                    ...
                    else if (r_vci_rsp_fifo_ins.wok()) // fifo not full
                    {
                        ...
                        vci_rsp_fifo_ins_put = TO BE COMPLETED vci_rsp_fifo_ins_data =
                            TO BE COMPLETED if (p_vci.reop.read())
                        {
            ...
            case RSP_INS_UNC: {
                if (p_vci.rspval.read())
                {
                    ...
                    {
                        vci_rsp_fifo_ins_put = TO BE COMPLETED vci_rsp_fifo_ins_data = TO BE COMPLETED r_vci_rsp_fsm =
                            RSP_IDLE;
                    }
                }
                break;
            ...
            case RSP_DATA_UNC: {
                if (p_vci.rspval.read())
                {
                    ...
                    {
                        vci_rsp_fifo_data_put = TO BE COMPLETED vci_rsp_fifo_data_data = TO BE COMPLETED r_vci_rsp_fsm =
                            RSP_IDLE;
                    }
                }
                break;
            ...
            r_vci_rsp_fifo_ins.update(vci_rsp_fifo_ins_get, vci_rsp_fifo_ins_put, vci_rsp_fifo_ins_data);

            r_vci_rsp_fifo_data.update(vci_rsp_fifo_data_get, vci_rsp_fifo_data_put, vci_rsp_fifo_data_data);
        ...
        tmpl(void)::genMoore()
        {
            ...
            case CMD_DATA_WRITE:
                ...
                p_vci.cmd     = vci_param::CMD_WRITE;
                p_vci.pktid   = 0;
                p_vci.trdid = TO BE COMPLETED p_vci.srcid = m_srcid;
                p_vci.cons                                = false;
                p_vci.wrap                                = false;
                p_vci.contig                              = true;
                p_vci.clen                                = 0;
