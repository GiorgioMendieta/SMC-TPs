...
#define tmpl(x) template <typename data_t> x AddressMaskingTable<data_t>
        ...
        tmpl(/**/)::AddressMaskingTable() { init(0, 0); }

        tmpl(/**/)::AddressMaskingTable(int use_bits, int drop_bits) { init(use_bits, drop_bits); }
        ...
        tmpl(/**/)::AddressMaskingTable(const AddressMaskingTable& ref) { init(ref.m_use_bits, ref.m_drop_bits); }
        ...
        {
            o << "<AMT: use=" << std::dec << m_use_bits << ", drop=" << m_drop_bits << ", mask=" << std::hex << mask()
              << ">" << std::endl;
        }

#undef tmpl

        template class AddressMaskingTable<uint32_t>;
