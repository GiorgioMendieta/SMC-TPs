...
namespace soclib
{
    namespace caba
    {
         ...
         * VCI Target port
         */
        template <typename vci_param> class VciTarget
        {
          public:
...
#define __ren(x) x((name + "_" #x).c_str())
            ...
            VciTarget(const std::string& name = sc_gen_unique_name("vci_target")) :
                __ren(rspack), __ren(rspval), __ren(rdata), __ren(reop), __ren(rerror), __ren(rsrcid), __ren(rtrdid),
                __ren(rpktid), __ren(cmdack), __ren(cmdval), __ren(address), __ren(be), __ren(cmd), __ren(contig),
                __ren(wdata), __ren(eop), __ren(cons), __ren(plen), __ren(wrap), __ren(cfixed), __ren(clen),
                __ren(srcid), __ren(trdid), __ren(pktid)
            {}
...
#undef __ren
            ...
            inline bool getAck() const { return rspack; }

            inline bool getVal() const { return cmdval; }

            inline void setAck(bool x) { cmdack = x; }

            inline void setVal(bool x) { rspval = x; }

            inline bool iProposed() const { return rspval; }

            inline bool iAccepted() const { return cmdval && cmdack; }

            inline bool peerAccepted() const { return rspval && rspack; }

            inline bool toPeerEnd() const { return peerAccepted() && reop; }

            void rspSetIds(typename vci_param::srcid_t srcid, typename vci_param::trdid_t trdid,
            ...
            inline void rspWrite(bool eop) { rspRead(eop, 0); }

            void rspNop() { rspval = false; }

            friend sc_core::sc_sensitive& operator<<(sc_core::sc_sensitive& ss, VciTarget<vci_param>& sig)
            {
                ss << sig.rspack << sig.cmdval << sig.address << sig.be << sig.cmd << sig.contig << sig.wdata << sig.eop
                   << sig.cons << sig.plen << sig.wrap << sig.cfixed << sig.clen << sig.srcid << sig.trdid << sig.pktid;
                return ss;
            }
        };
