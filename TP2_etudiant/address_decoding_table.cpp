...
#define tmpl(x) template <typename input_t, typename output_t> x AddressDecodingTable<input_t, output_t>
        ...
        tmpl(/**/)::AddressDecodingTable() { init(0, 0); }

        tmpl(/**/)::AddressDecodingTable(int use_bits, int drop_bits) { init(use_bits, drop_bits); }
        ...
        tmpl(void)::set(input_t where, output_t value) { m_table[id(where)] = value; }
        ...
        const AddressDecodingTable<input_t, output_t>& AddressDecodingTable<input_t, output_t>::operator=(
        ...
        tmpl(/**/)::~AddressDecodingTable() { delete[] m_table; }

        tmpl(void)::print(std::ostream& o) const
        {
            o << "<ADT: use=" << std::dec << m_use_bits << ", drop=" << m_drop_bits << ", mask=" << std::hex << mask()
              << std::endl;
            for (int i = 0; i < (1 << m_use_bits); ++i)
...
#undef tmpl
        ...
        template class AddressDecodingTable<uint32_t, bool>;
        template class AddressDecodingTable<uint32_t, int>;
