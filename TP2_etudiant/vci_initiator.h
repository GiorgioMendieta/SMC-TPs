...
namespace soclib
{
    namespace caba
    {
         ...
         * VCI Initiator port
         */
        template <typename vci_param> class VciInitiator
        {
          public:
...
#undef __ren
            ...
            inline bool getAck() const { return cmdack; }

            inline bool getVal() const { return rspval; }

            inline void setAck(bool x) { rspack = x; }

            inline void setVal(bool x) { cmdval = x; }

            inline bool iProposed() const { return cmdval; }

            inline bool iAccepted() const { return rspval && rspack; }

            inline bool peerAccepted() const { return cmdval && cmdack; }

            inline bool toPeerEnd() const { return peerAccepted() && eop; }

            void cmdNop() { cmdval = false; }

            friend sc_core::sc_sensitive& operator<<(sc_core::sc_sensitive& ss, VciInitiator<vci_param>& sig)
            {
                ss << sig.rspval << sig.rdata << sig.reop << sig.rerror << sig.rsrcid << sig.rtrdid << sig.rpktid
                   << sig.cmdack;
                return ss;
            }
        };
