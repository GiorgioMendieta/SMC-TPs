...
namespace soclib
{
    namespace caba
    {
         ...
         * VCI Initiator port
         */
        template <typename vci_param> class VciSignals
        {
          public:
...
#define ren(x) x(((std::string)(name_ + "_" #x)).c_str())
            ...
            VciSignals(std::string name_ = (std::string)sc_core::sc_gen_unique_name("vci")) :
                ren(rspack), ren(rspval), ren(rdata), ren(reop), ren(rerror), ren(rsrcid), ren(rtrdid), ren(rpktid),
                ren(cmdack), ren(cmdval), ren(address), ren(be), ren(cmd), ren(contig), ren(wdata), ren(eop), ren(cons),
                ren(plen), ren(wrap), ren(cfixed), ren(clen), ren(srcid), ren(trdid), ren(pktid)
            {}
