...
namespace soclib
{
    namespace caba
    {
        ...
        VciGcdMaster<vci_param>::VciGcdMaster(sc_module_name insname, const soclib::common::IntTab& index,
                                              const soclib::common::MappingTable& mt, const int seed,
                                              const typename vci_param::addr_t base) :
            sc_module(insname), r_fsm("r_fsm"), r_opa("r_opa"), r_opb("r_opb"), r_res("r_res"), r_cycle("r_cycle"),
            m_srcid(mt.indexForId(index)), m_base(base), p_resetn("p_resetn"), p_clk("p_clk"), p_vci("p_vci")
        {
        ...
        ////////////////////////////
        template <typename vci_param> VciGcdMaster<vci_param>::~VciGcdMaster() {}

        ////////////////////////////////
        template <typename vci_param> void VciGcdMaster<vci_param>::transition()
        {
            if (!p_resetn.read())
            {
                r_fsm = A COMPLETER r_cycle = 0;
                r_iter                      = 0;
                return;
            }
            ...
            case RANDOM:
                ...
                r_opa = A COMPLETER r_opb = A COMPLETER r_fsm = A COMPLETER break;
            case CMD_OPA:
            ...
            case RSP_STATUS:
                if (p_vci.rspval.read())
                {
                    if (p_vci.rdata.read() == 0)
                        r_fsm = A COMPLETER else r_fsm = A COMPLETER
                }
                break;
            case CMD_RESULT:
                if (p_vci.cmdack.read())
            ...
            case RSP_RESULT:
                if (p_vci.rspval.read())
                {
                    r_res = A COMPLETER r_fsm = A COMPLETER
                }
                break;
            case DISPLAY:
                r_fsm = A COMPLETER std::cout << std::dec << std::noshowbase;
        ...
        template <typename vci_param> void VciGcdMaster<vci_param>::genMoore()
        {
            ...
            case RANDOM:
            case DISPLAY:
                p_vci.cmdval = A COMPLETER p_vci.rspack = A COMPLETER break;
            case CMD_OPA:
                p_vci.cmdval  = true;
                ...
                break;
            case CMD_OPB:
                p_vci.cmdval = A COMPLETER p_vci.cmd = A COMPLETER p_vci.address = A COMPLETER p_vci.wdata =
                    A COMPLETER p_vci.rspack                                     = A COMPLETER break;
            case CMD_START:
                p_vci.cmdval = A COMPLETER p_vci.cmd = A COMPLETER p_vci.address = A COMPLETER p_vci.wdata =
                    A COMPLETER p_vci.rspack                                     = A COMPLETER break;
            case CMD_RESULT:
            ...
            case RSP_RESULT:
            case RSP_STATUS:
                p_vci.cmdval = A COMPLETER p_vci.cmd = A COMPLETER;
                p_vci.address = A COMPLETER p_vci.wdata = A COMPLETER p_vci.rspack = A COMPLETER break;
            } // end switch
        } // end genMoore()
