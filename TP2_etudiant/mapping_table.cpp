...
namespace soclib
{
    namespace common
    {
        ...
        MappingTable::MappingTable(size_t addr_width, const IntTab& level_addr_bits, const IntTab& level_id_bits,
                                   const addr_t cacheability_mask) :
            m_segment_list(), m_addr_width(addr_width), m_level_addr_bits(level_addr_bits),
            m_level_id_bits(level_id_bits), m_cacheability_mask(cacheability_mask)
        {
        ...
        MappingTable::MappingTable(const MappingTable& ref) :
            m_segment_list(ref.m_segment_list), m_addr_width(ref.m_addr_width),
            m_level_addr_bits(ref.m_level_addr_bits), m_level_id_bits(ref.m_level_id_bits),
            m_cacheability_mask(ref.m_cacheability_mask), m_rt_size(ref.m_rt_size)
        {}
        ...
        void MappingTable::add(const Segment& seg)
        {
            ...
            for (i = m_segment_list.begin(); i != m_segment_list.end(); i++)
            {
                Segment& s = *i;
                ...
                for (addr_t address = s.baseAddress() & ~(m_rt_size - 1);
                     (address < s.baseAddress() + s.size()) && (address >= (s.baseAddress() & ~(m_rt_size - 1)));
                     address += m_rt_size)
                {
                         ...
                         segaddress += m_rt_size)
                    {
                            ...
                            s.cacheable() != seg.cacheable())
                        {
                            ...
                            oss << "Segment " << s << " has a different cacheability attribute with same MSBs than "
                                << seg << std::endl;
        ...
        const std::list<Segment>& MappingTable::getAllSegmentList() const { return m_segment_list; }
        ...
        AddressDecodingTable<MappingTable::addr_t, bool> MappingTable::getCacheabilityTable() const
        {
            ...
            for (i = m_segment_list.begin(); i != m_segment_list.end(); i++)
            {
                for (addr_t addr = i->baseAddress() & ~(m_rt_size - 1);
                     (addr < i->baseAddress() + i->size()) && (addr >= (i->baseAddress() & ~(m_rt_size - 1)));
                     addr += m_rt_size)
                {
                    if (done[addr] && adt[addr] != i->cacheable())
                    {
                        ...
                        oss << "Incoherent Mapping Table:" << std::endl
                            << "Segment " << *i
                            << " has different cacheability than other segment with same masked address" << std::endl
        ...
        AddressDecodingTable<MappingTable::addr_t, bool> MappingTable::getLocalityTable(const IntTab& index) const
        {
            ...
            for (i = m_segment_list.begin(); i != m_segment_list.end(); i++)
            {
                     ...
                     addr += m_rt_size)
                {
                    ...
                    if (done[addr] && adt[addr] != val)
                    {
                        std::ostringstream oss;
                        oss << "Incoherent Mapping Table:" << std::endl
                            << "Segment " << *i << " targets different component than other segments with same MSBs"
                            << std::endl
                            << "Mapping table:" << std::endl
                            << *this;
                        throw soclib::exception::RunTimeError(oss.str());
        ...
        AddressDecodingTable<MappingTable::addr_t, int> MappingTable::getRoutingTable(const IntTab& index,
                                                                                      int default_index) const
        {
            ...
            {
#ifdef SOCLIB_MODULE_DEBUG
                std::cout << *i << ", m_rt_size=" << m_rt_size << ", m_rt_mask=" << ~(m_rt_size - 1) << std::endl;
#endif
...
#ifdef SOCLIB_MODULE_DEBUG
                std::cout << ' ' << (i->baseAddress() & ~(m_rt_size - 1)) << ' ' << (i->baseAddress() + i->size())
                     ...
                     addr += m_rt_size)
                {
                    ...
                    if (done[addr] && adt[addr] != val)
                    {
                        ...
                        oss << "Incoherent Mapping Table: for " << index << std::endl
                            << "Segment " << *i
                            << " targets different target (or cluster) than other segments with same routing bits"
                            << std::endl
                            << "Mapping table:" << std::endl
                            << *this;
                        throw soclib::exception::RunTimeError(oss.str());
        ...
        void MappingTable::print(std::ostream& o) const
        {
            ...
            o << "Mapping table: ad:" << m_level_addr_bits << " id:" << m_level_id_bits
              << " cacheability mask: " << std::hex << std::showbase << m_cacheability_mask << std::endl;
            for (i = m_segment_list.begin(); i != m_segment_list.end(); i++)
            {
                o << " " << (*i) << std::endl;
            }
        ...
        AddressDecodingTable<uint32_t, bool> MappingTable::getIdLocalityTable(const IntTab& index) const
        {
            ...
            for (i = m_segment_list.begin(); i != m_segment_list.end(); i++)
            {
                ...
                if (done[id] && adt[id] != val)
                {
                    std::ostringstream oss;
                    oss << "Incoherent Mapping Table:" << std::endl
                        << "Segment " << *i << " targets different component than other segments with same ID MSBs"
                        << std::endl
                        << "Mapping table:" << std::endl
                        << *this;
                    throw soclib::exception::RunTimeError(oss.str());
