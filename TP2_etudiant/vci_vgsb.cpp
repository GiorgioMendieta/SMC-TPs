...
namespace soclib
{
    namespace caba
    {
        ...
        VciVgsb<vci_param>::VciVgsb(sc_module_name name, MappingTable& maptab, size_t nb_master, size_t nb_slave) :
            sc_core::sc_module(name), m_routing_table(maptab.getRoutingTable(IntTab())), m_nb_initiator(nb_master),
            m_nb_target(nb_slave), p_clk("clk"), p_resetn("resetn"),
            p_to_initiator(
                soclib::common::alloc_elems<soclib::caba::VciTarget<vci_param>>("p_to_initiator", nb_master)),
            p_to_target(soclib::common::alloc_elems<soclib::caba::VciInitiator<vci_param>>("p_to_target", nb_slave)),
            r_fsm("r_fsm"), r_initiator_index("r_initiator_index"), r_target_index("r_target_index"),
            r_vci_counter(soclib::common::alloc_elems<sc_signal<uint32_t>>("r_vci_counter", nb_master, nb_slave))
        {
            ...
            dont_initialize();
            sensitive << p_clk.neg();
            for (size_t i = 0; i < nb_slave; i++)
                sensitive << p_to_target[i];

            SC_METHOD(genMealy_rspack);
            dont_initialize();
            sensitive << p_clk.neg();
            for (size_t i = 0; i < nb_master; i++)
        ...
        ////////////////////////////
        template <typename vci_param> VciVgsb<vci_param>::~VciVgsb()
        {
        ...
        ///////////////////////////
        template <typename vci_param> void VciVgsb<vci_param>::transition()
        {
            if (p_resetn == false)
            {
                r_fsm             = FSM_IDLE;
            ...
            case FSM_CMD:
                if (p_to_initiator[r_initiator_index.read()].eop.read() &&
                    ...
                    p_to_target[r_target_index.read()].cmdack.read())
                    r_fsm = FSM_RSP;
                break;

            case FSM_RSP:
                if (p_to_target[r_target_index.read()].reop && p_to_target[r_target_index.read()].rspval &&
        ...
        ////////////////////////////
        template <typename vci_param> void VciVgsb<vci_param>::genMealy_cmdval()
        {
            size_t ini = (size_t)r_initiator_index.read();
            size_t tgt = (size_t)r_target_index.read();

            if (r_fsm.read() == FSM_CMD)
            { // cmd packet transfer
        ...
        ////////////////////////////
        template <typename vci_param> void VciVgsb<vci_param>::genMealy_cmdack()
        {
            size_t ini = (size_t)r_initiator_index.read();
            size_t tgt = (size_t)r_target_index.read();
        ...
        ////////////////////////////
        template <typename vci_param> void VciVgsb<vci_param>::genMealy_rspval()
        {
            size_t ini = (size_t)r_initiator_index.read();
            size_t tgt = (size_t)r_target_index.read();

            if (r_fsm.read() == FSM_RSP)
        ...
        ////////////////////////////
        template <typename vci_param> void VciVgsb<vci_param>::genMealy_rspack()
        {
            size_t ini = (size_t)r_initiator_index.read();
            size_t tgt = (size_t)r_target_index.read();
