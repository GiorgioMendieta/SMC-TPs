...
namespace soclib
{
    namespace caba
    {
        ...
        template <typename vci_param> void VciGcdCoprocessor<vci_param>::transition()
        {
            if (!p_resetn.read())
            {
                r_vci_fsm = A COMPLETER r_exe_fsm = A COMPLETER return;
            }
            ...
            case EXE_IDLE:
                if (r_vci_fsm.read() == VCI_RSP_START)
                    r_exe_fsm = A COMPLETER break;
            case EXE_COMPARE:
                if (r_opa.read() < r_opb.read())
                    r_exe_fsm                      = A COMPLETER else if (r_opb.read() < r_opa.read()) r_exe_fsm =
                        A COMPLETER else r_exe_fsm = A COMPLETER break;
            case EXE_DECA:
                r_opa = A COMPLETER r_exe_fsm = A COMPLETER break;
            case EXE_DECB:
                r_opb = A COMPLETER r_exe_fsm = A COMPLETER break;
            } // end switch exe-fsm
            ...
            case VCI_GET_CMD:
                if (p_vci.cmdval.read())
                {
                    ...
                    r_srcid = A COMPLETER r_trdid = A COMPLETER r_pktid = A COMPLETER
                        // test the command
                    ...
                    {
                        r_opa = A COMPLETER r_vci_fsm = A COMPLETER
                    }
                    else if ((p_vci.cmd.read() == vci_param::CMD_WRITE) && (cell == GCD_OPB))
                    {
                        r_opb = A COMPLETER r_vci_fsm = A COMPLETER
                    }
        ...
        template <typename vci_param> void VciGcdCoprocessor<vci_param>::genMoore()
        {
            ...
            p_vci.rsrcid = A COMPLETER p_vci.rtrdid = A COMPLETER p_vci.rpktid = A COMPLETER p_vci.rerror =
                A COMPLETER p_vci.reop                                         = A COMPLETER

                switch (r_vci_fsm)
            {
            case VCI_GET_CMD:
                p_vci.cmdack = A COMPLETER p_vci.rspval = A COMPLETER p_vci.rdata = A COMPLETER break;
            case VCI_RSP_OPA:
            case VCI_RSP_OPB:
            case VCI_RSP_START:
                p_vci.cmdack = A COMPLETER p_vci.rspval = A COMPLETER p_vci.rdata = A COMPLETER break;
            case VCI_RSP_STATUS:
                p_vci.cmdack = A COMPLETER p_vci.rspval = A COMPLETER p_vci.rdata = A COMPLETER break;
            case VCI_RSP_RESULT:
                p_vci.cmdack = A COMPLETER p_vci.rspval = A COMPLETER p_vci.rdata = A COMPLETER
            }
        } // end genMoore()
