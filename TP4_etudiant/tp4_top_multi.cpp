...
#include "alloc_elems.h" // Needed for signal arrays allocation
...
#define SEG_DMA_SIZE 32 * NPROCS // 5 registers + padding
...
#define trdid_size  4 // Modified for multi processor
#define pktid_size  1
...
int _main(int argc, char* argv[])
{
    ...
    if (argc > 1)
    {
        for (int n = 1; n < argc; n = n + 2)
        {
            ...
            else if ((strcmp(argv[n], "-NPROCS") == 0) && (n + 1 < argc))
            {
                ...
                {
                    std::cout << "NPROCS must be between 1 and 4" << std::endl;
                    exit(1);
                }
            }
            else
    ...
    for (unsigned nproc = 0; nproc < nprocs; nproc++)
    {
        ...
        proc[nproc] = new VciXcacheWrapper<vci_param, Mips32ElIss>(name, nproc, maptab, IntTab(SRCID_PROC_BASE + nproc),
                                                                   icache_ways, icache_sets, icache_words, dcache_ways,
                                                                   dcache_sets, dcache_words);
    }
    ...
    dma = new VciMultiDma<vci_param>("dma", maptab, IntTab(SRCID_DMA), IntTab(TGTID_DMA), burst_size, nprocs);

    VciFrameBuffer<vci_param>* fbf;
    fbf = new VciFrameBuffer<vci_param>("fbf", IntTab(TGTID_FBF), maptab, FB_NPIXELS, FB_NLINES);

    // One IOC shared by all processors
    ...
    bus = new VciVgsb<vci_param>("bus", maptab, 2 + nprocs, 9); // IOC, DMA + nprocs initiators, 9 targets
    ...
    for (unsigned nproc = 0; nproc < nprocs; nproc++)
    {
        ...
        proc[nproc]->p_vci(signal_vci_init_proc[nproc]);
        proc[nproc]->p_irq[0](signal_irq_proc[nproc]);
        proc[nproc]->p_irq[1](signal_false);
        proc[nproc]->p_irq[2](signal_false);
        proc[nproc]->p_irq[3](signal_false);
        proc[nproc]->p_irq[4](signal_false);
        proc[nproc]->p_irq[5](signal_false);
    }
    ...
    for (int n = 1; n < ncycles; n++)
    {
        if (debug && (n > from_cycle))
        {
            ...
            for (unsigned i = 0; i < nprocs; i++)
            {
                ...
                if (signal_irq_proc[i].read())
                    std::cout << "IRQ_PROC_" << i << std::endl;
                if (signal_irq_dma[i].read())
                    std::cout << "IRQ_DMA_" << i << std::endl;
                if (signal_irq_tim[i].read())
                    std::cout << "IRQ_TIM_" << i << std::endl;
                if (signal_irq_tty[i].read())
                    std::cout << "IRQ_TTY_" << i << std::endl;
            }
        }
        sc_start(sc_time(1, SC_NS));
    }
