...
#define SEG_STACK_SIZE  0x01000000 * NPROCS
...
#define SEG_IOC_SIZE    0x00000040

#define SEG_DMA_BASE    0x93000000
#define SEG_DMA_SIZE    0x00000020 * NPROCS

#define SEG_FBF_BASE    0x96000000
#define SEG_FBF_SIZE    0x400000 + 0x4000
...
// SRCID definition
#define SRCID_PROC_BASE 0
#define SRCID_DMA       NPROCS
#define SRCID_IOC       NPROCS + 1
...
int _main(int argc, char *argv[])
{
        ...
        for( int n=1 ; n<argc ; n=n+2 )
        {
            ...
            else if( (strcmp(argv[n],"-NPROCS") == 0) && (n+1<argc) )
            {
                ...
                if(nprocs == 0 || nprocs > 4){
                    std::cout << "Please refer a strictly positive number of processors between 1 and 4" << std::endl;
                    exit(1);
                }
                std::cout << "Instanciating %d processors" << nprocs << std::endl;
            }
            else
            {
    ...
    for(int i=0; i<nprocs; i++){ 
        ...
        proc[i] = new VciXcacheWrapper<vci_param,Mips32ElIss>(name, i,maptab,IntTab(SRCID_PROC_BASE + i),
    ...
    for(int i=0; i<nprocs; i++){
        char name[16];
        sprintf(name, "tty_proc[%d]", i);
        tty_names.push_back(name);
    }
    
    std::cout << "Avant l'instanciation des compo Vci (tty)" << std::endl;
   
    VciMultiTty<vci_param>* tty;
    tty = new VciMultiTty<vci_param>("tty", IntTab(TGTID_TTY), maptab, tty_names);

    std::cout << "Avant l'instanciation des compo gcd" << std::endl;

    VciGcdCoprocessor<vci_param>* gcd;
    gcd = new VciGcdCoprocessor<vci_param>("gcd", IntTab(TGTID_GCD), maptab);

    std::cout << "Avant l'instanciation des compo timer" << std::endl;

    VciTimer<vci_param>* timer;
    timer = new VciTimer<vci_param>("timer", IntTab(TGTID_TIM), maptab, nprocs);

    std::cout << "Avant l'instanciation des compo multi_icu" << std::endl;

    VciMultiIcu<vci_param>* icu;
    icu = new VciMultiIcu<vci_param>("icu", IntTab(TGTID_ICU), maptab, 16, nprocs);

    std::cout << "Avant l'instanciation des compo multidma" << std::endl;

    VciMultiDma<vci_param>* dma;
    dma = new VciMultiDma<vci_param>("dma", maptab, IntTab(SRCID_DMA), IntTab(TGTID_DMA), 16, nprocs);

    std::cout << "Avant l'instanciation des compo fbf" << std::endl;

    VciFrameBuffer<vci_param>* fbf;
    fbf = new VciFrameBuffer<vci_param>("fbf", IntTab(TGTID_FBF), maptab, 128, 128);

    std::cout << "Avant l'instanciation des compo ioc" << std::endl;

    VciBlockDevice<vci_param>* ioc;
    ioc = new VciBlockDevice<vci_param>("ioc", maptab, IntTab(SRCID_IOC), IntTab(TGTID_IOC), ioc_filename); 

    std::cout << "Avant l'instanciation des compo bus" << std::endl;

    VciVgsb<vci_param>* bus;
    bus = new VciVgsb<vci_param>("bus", maptab, 2+nprocs, 9);

    std::cout << "Après l'instanciation des compo Vci" << std::endl;
    ...
    std::cout << "Après netlist des proc" << std::endl;
    ...
    std::cout << "Après netlist tty" << std::endl;
    ...
    std::cout << "Après netlist timer" << std::endl;
    ...
    std::cout << "Après netlist icu 0 à 3" << std::endl;
    ...
    std::cout << "Après netlist icu de dma timer et tty" << std::endl;

    for(int i=nprocs; i<4; i++){
        icu->p_irq_in[4+i] (signal_false);
        icu->p_irq_in[8+i] (signal_false);
        icu->p_irq_in[12+i] (signal_false);
    }

    std::cout << "Après netlist ICU" << std::endl;
    ...
    std::cout << "Après les bus p_to_initiator de proc" << std::endl;

    bus->p_to_initiator[SRCID_DMA]  (signal_vci_init_dma);
    bus->p_to_initiator[SRCID_IOC]  (signal_vci_init_ioc);

    std::cout << "Après les bus p_to_initiator du bus" << std::endl;
    ...
    bus->p_to_target[TGTID_IOC]     (signal_vci_tgt_ioc);
     ...
     std::cout << "Après les bus p_to_target" << std::endl;
    ...
    signal_resetn = true;
    std::cout << "Avant la boucle for de la simulation" << std::endl;
    ...
    for ( int n=1 ; n<ncycles ; n++ )
    {
        if( debug && (n > from_cycle) )
        {
            ...
            for(int i=0; i<nprocs; i++){
                ...
                if( signal_irq_proc[i].read() ) std::cout << "IRQ_PROC" << std::endl;
                if( signal_irq_tim[i].read() )  std::cout << "IRQ_TIM"  << std::endl;
                if( signal_irq_tty[i].read() )  std::cout << "IRQ_TTY"  << std::endl;
                if( signal_irq_dma[i].read() )  std::cout << "IRQ_DMA"  << std::endl;
            }
    ...
    sc_stop();
    std::cout << "Fin de _main de topcpp4" << std::endl;
    return(0);
...
{
    std::cout << "Début du sc_main" << std::endl;
    try {
